...
def inferir_categoria_y_temas(texto_art1):
    ...
    if any(kw in texto for kw in ["transporte", "emtupse", "pasajeros", "línea", "recorrido", "micro centro"]):
        return "D", CATEGORIAS_DIGESTO_COMPLETO["D"], ["transporte urbano", "servicio nocturno", "jóvenes", "EMTUPSE", "boliches"]
    if any(kw in texto for kw in ["impuesto", "tasa", "tributo", "factura", "fiscal"]):
        return "J", CATEGORIAS_DIGESTO_COMPLETO["J"], ["impuestos", "tasas municipales", "facturación"]
    if any(kw in texto for kw in ["tránsito", "vehículo", "conducir", "ruta"]):
        return "C", CATEGORIAS_DIGESTO_COMPLETO["C"], ["tránsito", "reglamentación vial"]
...
def extraer_palabras_clave(texto_completo: str, art1: str) -> list:
    ...
    for nombre in nombres_propios:
        ...
        if nombre not in ["Villa María", "Concejo Deliberante", "Departamento Ejecutivo"]:
    ...
    for sigla in siglas:
        ...
        if sigla not in ["EL", "LA", "DE", "CON", "PARA", "QUE", "VILLA", "MARIA", "ART"]:
    ...
    ordenanzas_ref = re.findall(r'[Oo]rdenanza\s*[Nn]?[°º]?\s*(\d{4,5})', texto_analisis)
    ...
    palabras_clave_filtradas = [
        pc for pc in palabras_clave 
        if 3 <= len(pc) <= 50 and pc.strip()
    ]
...
def extraer_texto_pdf(ruta_pdf):
    ...
    try:
        imagenes = convert_from_path(ruta_pdf, poppler_path=r"C:\poppler\Library\bin", dpi=200)
        texto_ocr = "".join(pytesseract.image_to_string(img, lang='spa') + "\n" for img in imagenes)
...
def extraer_metadatos_ordenanza(texto):
    ...
    for pattern in patterns_numero:
        match = re.search(pattern, texto[:500], re.IGNORECASE)  # Buscar en primeros 500 chars
        if match:
    ...
    anios_texto = {
        ...
        "dos mil nueve": "2009", "dos mil diez": "2010", "dos mil once": "2011",
        "dos mil doce": "2012", "dos mil trece": "2013", "dos mil catorce": "2014",
        "dos mil quince": "2015", "dos mil dieciseis": "2016", "dos mil dieciséis": "2016",
        "dos mil diecisiete": "2017", "dos mil dieciocho": "2018", "dos mil diecinueve": "2019",
        "dos mil veinte": "2020", "dos mil veintiuno": "2021", "dos mil veintidos": "2022",
        "dos mil veintidós": "2022", "dos mil veintitres": "2023", "dos mil veintitrés": "2023",
        "dos mil veinticuatro": "2024", "dos mil veinticinco": "2025", "dos mil veintiseis": "2026",
    ...
    if match_slash:
        ...
        try:
            if 1 <= int(dia) <= 31 and 1 <= int(mes) <= 12 and 1900 <= int(anio) <= 2030:
...
def dividir_en_chunks_mejorado(texto, max_len=MAX_CHUNK_LEN):
    articulos = re.split(r'(Art(?:ículo|iculo|\.)?\s*\d+[º°]?\s*[-–—]?)', texto, flags=re.IGNORECASE)
    if len(articulos) > 2:
        chunks = []
...
def combinar_textos_y_metadatos(archivos_grupo):
    """
    Combina información de múltiples versiones del mismo documento
    Prioriza el documento con mejor calidad de extracción
    """
    candidatos = []
    ...
    for archivo in archivos_grupo:
        ...
        print(f"    📄 {archivo}: score={candidato['score']}, num={num}, fecha={fecha}")
    ...
    print(f"  ✓ Mejor archivo: {mejor['archivo']} (score: {mejor['score']})")
    ...
    return mejor["archivo"], mejor["texto"], {
...
def procesar_grupo_ordenanza(numero_ord, lista_archivos):
    resultado = combinar_textos_y_metadatos(lista_archivos)
    if not resultado[0]:
        print(f"⚠️ No se pudo procesar la ordenanza {numero_ord}")
        return [], [], []

    nombre_archivo, texto_base, metadatos_base = resultado
    ...
    print(f"    🔑 Palabras clave: {', '.join(palabras_clave[:5])}{'...' if len(palabras_clave) > 5 else ''}")
    ...
    with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
        futures = {executor.submit(generar_embedding_local, chunk): chunk for chunk in chunks_base}
    ...
    return metadatos, chunks_base, embeddings
...
def cargar_datos_existentes():
    ...
    if os.path.exists(metadatos_path):
        ...
        print(f"✓ Cargados {len(metadatos)} metadatos expandidos desde {len(metadatos_comprimidos)} ordenanzas")
    ...
    if os.path.exists(index_path):
        ...
        embeddings_existentes = [index_temp.reconstruct(i) for i in range(index_temp.ntotal)]
...
def obtener_ordenanzas_procesadas(metadatos):
    ...
    return set(m.get("numero_ordenanza") for m in metadatos if m.get("numero_ordenanza") != "desconocido")
...
def guardar_checkpoint(metadatos, chunks, embeddings):
    ...
    metadatos_lista = [
        {**v, "numero_ordenanza": k} 
        for k, v in sorted(metadatos_comprimidos.items())
    ]
...
def procesar_todos_documentos(inicio=None, fin=None, guardar_cada=50):
    ...
    if ordenanzas_procesadas:
        ...
        print(f"   Rango: {min(map(int, [o for o in ordenanzas_procesadas if o.isdigit()]))} - {max(map(int, [o for o in ordenanzas_procesadas if o.isdigit()]))}")
    ...
    numeros_ordenanza = [n for n in numeros_ordenanza if str(n) not in ordenanzas_procesadas]
    ...
    for idx, num in enumerate(numeros_ordenanza, 1):
        try:
            print(f"📦 ORDENANZA {num} ({idx}/{len(numeros_ordenanza)}) ⬅ Procesando de atrás hacia adelante")
            archivos = grupos[str(num)]
            
            m, c, e = procesar_grupo_ordenanza(str(num), archivos)
