...
//           <h2 className="font-semibold mb-2 text-gray-800">📄 Documentos encontrados</h2>
...
//             Respuesta
//           </h2>
//           <p className="text-gray-700 leading-relaxed mb-4">{respuesta}</p>
...
//             >
//               {mostrarDocs ? "Ocultar documentos" : `Ver ${documentos.length} documentos`}
//             </button>
//           )}
//         </div>
//       )}
...
//             <div key={i} className="bg-white/95 backdrop-blur-sm rounded-xl shadow p-5 hover:shadow-md transition">
//               <div className="flex justify-between items-start mb-3">
//                 <div>
//                   <p className="font-semibold text-gray-800">
//                     {doc.numero_ordenanza ? `Ordenanza N° ${doc.numero_ordenanza}` : doc.nombre}
//                   </p>
//                   {doc.fecha_sancion && (
...
//                 )}
//               </div>
//               <p className="text-sm text-gray-600 leading-relaxed">{doc.fragmento}</p>
//             </div>
//           ))}
//         </div>
//       )}
...
function BuscadorIA() {
  const [busqueda, setBusqueda] = useState("");
  // Estados comentados porque no se usan, pero puedes dejarlos si planeas reactivar luego
  // const [respuesta, setRespuesta] = useState<string | null>(null);
  // const [documentos, setDocumentos] = useState<any[]>([]);
  // const [mostrarDocs, setMostrarDocs] = useState(false);
  // const [cargando, setCargando] = useState(false);
  ...
  // La funcionalidad está deshabilitada, así que no se necesita lógica de búsqueda
  // const handleBuscarStream = async () => { ... };

  return (
    <div className="w-full max-w-4xl mx-auto">
      {/* Barra de búsqueda deshabilitada */}
      <div className="bg-gray-100 rounded-2xl shadow-lg p-2 mb-4 opacity-75">
        <div className="flex gap-2">
          <input
            type="text"
            value={busqueda}
            ...
            placeholder="Pregunta sobre las ordenanzas municipales..."
            className="flex-1 px-6 py-4 bg-transparent text-gray-500 placeholder-gray-400 focus:outline-none text-lg cursor-not-allowed"
            disabled
          />
          <button
            className="px-8 py-4 bg-gray-400 text-gray-600 rounded-xl font-medium cursor-not-allowed"
            disabled
          >
            Buscar
          </button>
  ...
  <div className="inline-block px-6 py-3 bg-pink-100 border-2 border-red-500 text-red-800 font-medium rounded-xl shadow-md">
    ⚠️ Momentáneamente fuera de servicio
  </div>
</div>

      {/* El resto del componente (respuesta, documentos, etc.) se omite porque no se usa */}
    </div>
  );
}
