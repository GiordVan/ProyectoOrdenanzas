...
import { Filter, X, FileText, Calendar, AlertCircle, MoveUp } from "lucide-react";
...
function BuscadorManual() {
  ...
  const cargarMetadatos = async () => {
    try {
      ...
      console.log(`✅ Cargadas ${ordenanzasUnicas.length} ordenanzas desde API`);
    } catch (error) {
  ...
  const cargarConPaginacion = async () => {
    try {
      const response = await fetch(`${API_URL}/metadatos-paginado?pagina=1&por_pagina=100`);
      ...
      console.log(`✅ Modo paginación: ${data.metadatos.length} de ${data.total} ordenanzas cargadas`);
    } catch (error) {
  ...
  const aplicarFiltros = () => {
    ...
    if (fechaDesde) {
      resultados = resultados.filter(ord => ord.fecha_sancion_iso >= fechaDesde);
    }

    if (fechaHasta) {
      resultados = resultados.filter(ord => ord.fecha_sancion_iso <= fechaHasta);
    }
    ...
    if (estadoVigencia) {
      resultados = resultados.filter(ord => ord.estado_vigencia === estadoVigencia);
    }
  ...
  const cargarMasItems = useCallback(() => {
    ...
    setCargandoMas(true);
    setTimeout(() => {
      setItemsVisibles(prev => Math.min(prev + ITEMS_POR_PAGINA, filtradas.length));
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !cargandoMas && itemsVisibles < filtradas.length) {
          cargarMasItems();
        }
      },
      { threshold: 0.1 }
        ...
        <div className="w-full lg:w-80 lg:flex-shrink-0">
          ...
          <div className="mt-4 text-xs text-gray-500 text-center">
            {cargando
              ...
              : `${Math.min(itemsVisibles, filtradas.length)} de ${filtradas.length} resultados`}
          </div>
        </div>
      </div>
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6 flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-red-800 font-semibold mb-1">Error al cargar datos</p>
        ...
        <div className="space-y-3">
          {cargando ? (
            ...
            <>
                ...
                <div ref={observerTarget} className="py-8 text-center">
                  {cargandoMas && (
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <p className="text-gray-500">Cargando más ordenanzas...</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {itemsVisibles >= filtradas.length && filtradas.length > ITEMS_POR_PAGINA && (
